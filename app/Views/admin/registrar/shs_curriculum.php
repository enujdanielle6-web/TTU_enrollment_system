...
<main class="py-5 bg-light min-vh-100">
  <div class="container-fluid px-lg-5">
        ...
        <div class="d-flex align-items-center gap-3">
          ...
          <div>
            <div class="d-flex align-items-center gap-2 mb-1 flex-wrap">
              <h1 class="h4 fw-bold text-dark mb-0">SHS Curriculum Management</h1>
              ...
              <span class="badge bg-light text-secondary border rounded-pill px-2.5 py-0.5 small fw-semibold">
                <i class="bi bi-layers text-info me-1"></i><?= count($curricula ?? []) ?> Plans
              </span>
            </div>
            <p class="text-muted small mb-0">Manage academic curricula and tracks for Senior High School strands.</p>
          </div>
        </div>
        <div class="d-flex align-items-center gap-2">
          <button class="btn btn-primary rounded-pill px-3 py-2 fw-medium shadow-sm d-inline-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#createCurriculumModal">
    ...
    <!-- Curriculum Directory Card (Dossier Card Styling) -->
    <div class="dossier-card mb-4 fade-in-up" style="animation-delay: 0.15s;">
      <div class="dossier-card-header d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
        <div class="d-flex align-items-center gap-2.5">
          ...
          <div>
            <h2 class="h5 fw-bold text-dark mb-0 d-inline-block align-middle">Curriculum Directory</h2>
            <span class="badge bg-light text-secondary border rounded-pill px-2.5 py-1 small fw-semibold ms-2 align-middle">
              <?= count($curricula ?? []) ?> Records
            </span>
          </div>
        </div>
        <div>
          <div class="input-group input-group-sm" style="width: 220px;">
            ...
            <input type="text" id="tableSearch" class="form-control form-control-sm border-start-0 ps-0" placeholder="Search curriculum...">
          </div>
        </div>
      </div>
      <div class="p-0">
              ...
              <?php if (empty($curriculaData)): ?>
                <tr>
                  <td colspan="8" class="text-center py-5 text-muted">
                    ...
                    No SHS curricula found.
                  </td>
                </tr>
              <?php else: ?>
                <?php foreach ($curriculaData as $cur): 
                  $totalUsage = (int)($cur['section_count'] ?? 0);
                ?>
                  <tr>
                    <td class="ps-4 fw-bold text-dark"><?= htmlspecialchars($cur['strand_code'], ENT_QUOTES, 'UTF-8') ?></td>
                    <td>
                      <div class="fw-medium text-dark"><?= htmlspecialchars($cur['curriculum_name'], ENT_QUOTES, 'UTF-8') ?></div>
                      <?php if ($totalUsage > 0): ?>
                        <small class="text-muted"><i class="bi bi-people me-1"></i><?= $totalUsage ?> active section(s)</small>
                      <?php else: ?>
                        <small class="text-muted">Unused</small>
                      <?php endif; ?>
                    </td>
                    <td><span class="badge bg-light text-dark border">v<?= htmlspecialchars($cur['version'], ENT_QUOTES, 'UTF-8') ?></span></td>
                    ...
                    <td>
                      ...
                      <?php elseif ($cur['status'] === 'draft'): ?>
                        <span class="badge bg-warning bg-opacity-10 text-warning rounded-pill px-3 py-2">
                          <i class="bi bi-pencil-fill me-1"></i>Draft
                        </span>
                      <?php elseif ($cur['status'] === 'archived'): ?>
                      ...
                      <?php else: ?>
                        <span class="badge bg-dark bg-opacity-10 text-muted rounded-pill px-3 py-2">
                          <i class="bi bi-x-circle me-1"></i><?= htmlspecialchars(ucfirst($cur['status']), ENT_QUOTES, 'UTF-8') ?>
                        </span>
                      <?php endif; ?>
                    </td>
                    <td><span class="badge bg-secondary rounded-pill px-3"><?= esc($cur['total_subjects']) ?></span></td>
                    <td><span class="fw-semibold text-primary"><?= esc($cur['total_units'] ?? 0) ?> units</span></td>
                    <td class="text-end pe-4">
                      <?php if ($cur['status'] === 'draft'): ?>
                        ...
                        <button type="button" class="btn btn-sm btn-outline-secondary rounded-pill px-3 me-1" 
                                onclick="openEditCurriculum(<?= esc($cur['curriculum_id']) ?>, <?= esc($cur['strand_id']) ?>, '<?= htmlspecialchars(addslashes($cur['curriculum_name']), ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars(addslashes($cur['version']), ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars(addslashes($cur['effective_academic_year'] ?? ''), ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars(addslashes($cur['description'] ?? ''), ENT_QUOTES, 'UTF-8') ?>')"
                                title="Edit Draft Details">
                          <i class="bi bi-pencil-fill me-1"></i> Edit
                        </button>
                        <a href="shs_curriculum_builder.php?curriculum_id=<?= esc($cur['curriculum_id']) ?>" class="btn btn-sm btn-primary rounded-pill px-3 me-1" title="Manage Subjects in Builder">
                          <i class="bi bi-tools me-1"></i> Manage Subjects
                        </a>
                        <button type="button" class="btn btn-sm btn-success rounded-pill px-3 me-1" 
                                onclick="openActivateModal(<?= esc($cur['curriculum_id']) ?>, '<?= htmlspecialchars(addslashes($cur['curriculum_name']), ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars(addslashes($cur['version']), ENT_QUOTES, 'UTF-8') ?>')" title="Activate curriculum for official enrollment">
                          <i class="bi bi-check-circle me-1"></i> Activate
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-danger rounded-pill px-2" 
                                data-bs-toggle="modal" 
                                data-bs-target="#deleteCurriculumModal"
                                onclick="setDeleteCurriculum(<?= esc($cur['curriculum_id']) ?>, '<?= htmlspecialchars(addslashes($cur['curriculum_name']), ENT_QUOTES, 'UTF-8') ?>')"
                                title="Delete Unused Draft">
                          <i class="bi bi-trash-fill"></i>
                        </button>
                      <?php elseif ($cur['status'] === 'active'): ?>
                        <!-- ACTIVE ACTIONS: View, Create New Version, Archive -->
                        <a href="shs_curriculum_builder.php?curriculum_id=<?= esc($cur['curriculum_id']) ?>" class="btn btn-sm btn-outline-primary rounded-pill px-3 me-1" title="View curriculum catalog (Read-only)">
                          <i class="bi bi-eye me-1"></i> View
                        </a>
                        <button type="button" class="btn btn-sm btn-primary rounded-pill px-3 me-1" 
                                onclick="openCloneModal(<?= esc($cur['curriculum_id']) ?>, '<?= htmlspecialchars(addslashes($cur['curriculum_name']), ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars(addslashes($cur['version']), ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars(addslashes($cur['effective_academic_year'] ?? ''), ENT_QUOTES, 'UTF-8') ?>')" title="Create a new draft version by cloning this active curriculum">
                          <i class="bi bi-files me-1"></i> Create New Version
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-secondary rounded-pill px-3" 
                                onclick="openArchiveModal(<?= esc($cur['curriculum_id']) ?>, '<?= htmlspecialchars(addslashes($cur['curriculum_name']), ENT_QUOTES, 'UTF-8') ?>')" title="Retire and archive curriculum">
                          <i class="bi bi-archive me-1"></i> Archive
                        </button>
                      <?php else: /* ARCHIVED / INACTIVE */ ?>
                        <!-- ARCHIVED ACTIONS: View only -->
                        <a href="shs_curriculum_builder.php?curriculum_id=<?= esc($cur['curriculum_id']) ?>" class="btn btn-sm btn-outline-secondary rounded-pill px-3" title="View historical curriculum (Read-only)">
                          <i class="bi bi-eye me-1"></i> View
                        </a>
                      <?php endif; ?>
                    </td>
    ...
    <div class="modal-content border-0 shadow">
      <form action="shs_curriculum_process.php" method="POST">
        ...
        <div class="modal-header bg-light border-bottom-0 pb-3">
          <h5 class="modal-title fw-bold text-dark"><i class="bi bi-plus-circle-fill text-primary me-2"></i>Create New Draft SHS Curriculum</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body p-4 pt-2">
            <div class="mb-3">
                <label class="form-label small fw-semibold text-dark">SHS Strand <span class="text-danger">*</span></label>
                <select class="form-select bg-light" name="strand_id" required>
                <option value="" disabled selected>Select Strand</option>
                <?php foreach ($activeStrands as $strand): ?>
                    <option value="<?= esc($strand['id']) ?>"><?= htmlspecialchars($strand['code'], ENT_QUOTES, 'UTF-8') ?> - <?= htmlspecialchars($strand['name'], ENT_QUOTES, 'UTF-8') ?></option>
                <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-3">
                <label class="form-label small fw-semibold text-dark">Curriculum Name <span class="text-danger">*</span></label>
                <input type="text" class="form-control bg-light" name="curriculum_name" placeholder="e.g. STEM 2026 Standard Track" required>
            </div>
            <div class="row mb-3">
                <div class="col-6">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control bg-light" name="description" rows="2" placeholder="Curriculum overview and track notes..."></textarea>
            </div>
            <div class="alert alert-info py-2 px-3 small mb-0">
              <i class="bi bi-info-circle me-1"></i> New curricula start in <strong>Draft</strong> status. You can customize the subjects in the Builder before activating it.
            </div>
        </div>
    ...
    <div class="modal-content border-0 shadow">
      <form action="shs_curriculum_process.php" method="POST">
        ...
        <div class="modal-body p-4 pt-2">
            <div class="mb-3">
                <label class="form-label small fw-semibold text-dark">SHS Strand <span class="text-danger">*</span></label>
                <select class="form-select bg-light" name="strand_id" id="edit_curr_strand_id" required>
                <?php foreach ($activeStrands as $strand): ?>
                    <option value="<?= esc($strand['id']) ?>"><?= htmlspecialchars($strand['code'], ENT_QUOTES, 'UTF-8') ?> - <?= htmlspecialchars($strand['name'], ENT_QUOTES, 'UTF-8') ?></option>
                <?php endforeach; ?>
                </select>
      ...
      <form action="shs_curriculum_process.php" method="POST">
        ...
        <div class="modal-body p-4 pt-3">
            ...
            <p class="small text-muted mb-0">
              <i class="bi bi-check2-all text-success me-1"></i> All mapped subjects from the source curriculum will be duplicated into the new <strong>Draft</strong> version.
            </p>
        </div>
        <div class="modal-footer border-top-0 pt-0 px-4 pb-4">
          <button type="button" class="btn btn-light px-4 rounded-pill fw-medium" data-bs-dismiss="modal">Cancel</button>
      ...
      <form action="shs_curriculum_process.php" method="POST">
        ...
        <div class="modal-body p-4 text-center">
            ...
            <p class="text-muted small mb-0">
              Archived curricula are retired from new admissions, but will be preserved permanently for existing students and transcripts.
            </p>
        </div>
        <div class="modal-footer border-top-0 d-flex justify-content-center pb-4">
          <button type="button" class="btn btn-light px-4 rounded-pill fw-medium" data-bs-dismiss="modal">Cancel</button>
...
<div class="modal fade" id="deleteCurriculumModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow">
      ...
      <div class="modal-footer border-top-0 d-flex justify-content-center pb-4">
        <button type="button" class="btn btn-light px-4 rounded-pill fw-medium" data-bs-dismiss="modal">Cancel</button>
        <form action="shs_curriculum_process.php" method="POST" class="d-inline">
            <?= getCsrfInput() ?>
...
function openEditCurriculum(id, strandId, name, version, ay, desc) {
    document.getElementById('edit_curr_id').value = id;
    document.getElementById('edit_curr_strand_id').value = strandId;
