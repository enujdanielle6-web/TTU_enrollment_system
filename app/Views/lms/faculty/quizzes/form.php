...
<div class="container py-5">
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-header bg-white border-bottom p-4">
            <h4 class="mb-0 fw-bold"><?= esc($quiz ? 'Edit Quiz Settings' : 'Create Quiz') ?></h4>
        </div>
        <div class="card-body p-4">
            <form action="/sia/lms/faculty/course/<?= esc($course['lms_course_id']) ?>/quizzes/<?= esc($quiz ? $quiz['id'] . '/update' : 'store') ?>" method="POST">
                ...
                <div class="row mb-4 g-3">
                    <div class="col-md-4">
                        ...
                        <input type="number" name="time_limit" class="form-control" value="<?= esc($quiz ? $quiz['time_limit'] : '') ?>" placeholder="Leave blank for unlimited" min="1">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold">Max Attempts</label>
                        <input type="number" name="max_attempts" class="form-control" value="<?= esc($quiz ? $quiz['max_attempts'] : 1) ?>" min="1">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-bold">Passing Score (%)</label>
                        <input type="number" name="passing_score" class="form-control" value="<?= esc($quiz ? $quiz['passing_score'] : '') ?>" placeholder="Optional" min="0" max="100">
                    </div>
                </div>
                ...
                <div class="row mb-4 g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="status" class="form-select">
                            <option value="draft" <?= esc(($quiz && $quiz['status'] === 'draft') ? 'selected' : '') ?>>Draft (Hidden)</option>
                            <option value="published" <?= esc(($quiz && $quiz['status'] === 'published') ? 'selected' : '') ?>>Published</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end gap-2">
                    <a href="/sia/lms/faculty/course/<?= esc($course['lms_course_id']) ?>/quizzes" class="btn btn-light">Cancel</a>
                    <button type="submit" class="btn btn-primary px-4"><?= esc($quiz ? 'Save Settings' : 'Create Quiz') ?></button>
                </div>
            </form>
        </div>
    </div>
