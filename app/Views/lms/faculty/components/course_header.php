<?php
/**
 * Faculty Course Header & Horizontal Navigation
 *
 * Variables expected:
...
if (!isset($assignment_count)) {
    try {
        ...
        $assignment_count = count($svc->getAssignmentsByCourse($courseId, false));
    } catch (\Throwable $e) {
        $assignment_count = 0;
    }
}
...
if (!isset($quiz_count)) {
    try {
        ...
        $quiz_count = count($qSvc->getQuizzesByCourse($courseId, false));
    } catch (\Throwable $e) {
        $quiz_count = 0;
    }
}
...
if (!isset($announcement_count)) {
    try {
        ...
        $announcement_count = count($aSvc->getCourseAnnouncements($courseId, false));
    } catch (\Throwable $e) {
...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 align-items-center">
            <li class="breadcrumb-item">
                <a href="/sia/lms/faculty/dashboard.php" class="text-decoration-none text-muted">
    ...
    <a href="/sia/lms/faculty/dashboard.php" class="btn btn-outline-secondary btn-sm rounded-pill px-3 fw-semibold d-inline-flex align-items-center gap-2">
        <i class="bi bi-arrow-left"></i> Back to Dashboard
    </a>
</div>

<!-- Course Banner -->
<div class="lms-banner mb-3 text-white p-4 p-md-5 rounded-4 shadow-sm position-relative overflow-hidden" style="background: linear-gradient(135deg, #0d6efd 0%, #0a58ca 100%);">
    ...
    <div class="position-relative z-1 row align-items-center g-3">
        <div class="col-md-8">
            <div class="d-flex align-items-center gap-2 mb-2 flex-wrap">
                ...
                <span class="badge bg-white bg-opacity-20 text-white px-3 py-2 rounded-pill fw-semibold">
                    <i class="bi bi-person-badge me-1"></i> Instructor View
                </span>
            </div>
        ...
        <div class="col-md-4 text-md-end mt-3 mt-md-0">
            <div class="bg-white text-dark p-3 rounded-4 shadow-sm d-inline-block text-start">
                <p class="small text-muted fw-bold text-uppercase mb-1">Teaching Status</p>
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div>
                        <h6 class="mb-0 fw-bold">Active Course</h6>
                        <small class="text-muted">Section <?= htmlspecialchars($sectionCode) ?></small>
                    </div>
                </div>
            </div>
        </div>
...
<!-- Course Horizontal Navigation Bar -->
<div class="course-nav-container mb-4">
    <a href="/sia/lms/faculty/course.php?id=<?= esc($courseId) ?>" 
       class="course-nav-link <?= esc($activeTab === 'modules' ? 'active' : '') ?>">
        <i class="bi bi-folder2-open"></i>
        <span>Modules &amp; Materials</span>
    </a>

    <a href="/sia/lms/faculty/course/<?= esc($courseId) ?>/announcements" 
    ...
    <a href="/sia/lms/faculty/course/<?= esc($courseId) ?>/assignments" 
    ...
    <a href="/sia/lms/faculty/course/<?= esc($courseId) ?>/quizzes" 
    ...
    <a href="/sia/lms/faculty/course/<?= esc($courseId) ?>/gradebook" 
       class="course-nav-link <?= esc($activeTab === 'gradebook' ? 'active' : '') ?>">
        <i class="bi bi-star"></i>
        <span>Grades</span>
    </a>

    <a href="/sia/lms/faculty/course/<?= esc($courseId) ?>/attendance" 
