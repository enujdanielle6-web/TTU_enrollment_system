...
<aside class="lms-sidebar" id="lmsSidebar">
    ...
    <a href="/sia/lms/student/dashboard.php" class="lms-nav-link <?= esc($current_page == 'dashboard.php' ? 'active' : '') ?>">
        <i class="bi bi-grid-1x2-fill"></i>
        <span class="nav-text">Dashboard</span>
    </a>
    <a href="/sia/lms/student/my_courses.php" class="lms-nav-link <?= esc($current_page == 'my_courses.php' ? 'active' : '') ?>">
    ...
    <?php if (isset($course) && isset($course['lms_course_id'])): ?>
        ...
        <a href="/sia/lms/student/course.php?id=<?= esc($course['lms_course_id']) ?>" class="lms-nav-link <?= esc(strpos($current_page, 'course.php') !== false ? 'active' : '') ?>">
            <i class="bi bi-folder2-open"></i>
            <span class="nav-text">Modules & Materials</span>
        </a>
        <a href="/sia/lms/student/course/<?= esc($course['lms_course_id']) ?>/announcements" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], '/announcements') !== false ? 'active' : '') ?>">
            <i class="bi bi-megaphone"></i>
            <span class="nav-text">Announcements</span>
        </a>
        <a href="/sia/lms/student/course/<?= esc($course['lms_course_id']) ?>/assignments" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], '/assignments') !== false ? 'active' : '') ?>">
            <i class="bi bi-journal-text"></i>
            <span class="nav-text">Assignments</span>
        </a>
        <a href="/sia/lms/student/course/<?= esc($course['lms_course_id']) ?>/quizzes" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], '/quizzes') !== false ? 'active' : '') ?>">
            <i class="bi bi-pencil-square"></i>
            <span class="nav-text">Online Quizzes</span>
        </a>
        <a href="/sia/lms/student/course/<?= esc($course['lms_course_id']) ?>/gradebook" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], '/gradebook') !== false ? 'active' : '') ?>">
            <i class="bi bi-star-fill"></i>
            <span class="nav-text">Grades</span>
        </a>
        <a href="/sia/lms/student/course/<?= esc($course['lms_course_id']) ?>/attendance" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], '/attendance') !== false ? 'active' : '') ?>">
            <i class="bi bi-person-check-fill"></i>
            <span class="nav-text">Attendance</span>
        </a>
    <?php endif; ?>
    <hr class="text-secondary mx-3 my-2">
    <a href="/sia/lms/student/calendar" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], '/calendar') !== false ? 'active' : '') ?>">
        <i class="bi bi-calendar-event-fill"></i>
        <span class="nav-text">Calendar</span>
    </a>
    <a href="/sia/lms/student/messages.php" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], 'messages.php') !== false ? 'active' : '') ?>">
        <i class="bi bi-chat-dots-fill"></i>
        <span class="nav-text">Messages / Forums</span>
    </a>
    <a href="/sia/lms/student/profile.php" class="lms-nav-link <?= esc(strpos($_SERVER['REQUEST_URI'], 'profile.php') !== false ? 'active' : '') ?>">
