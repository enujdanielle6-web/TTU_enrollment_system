                            ...
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    ...
                                    <p class="mb-1 text-muted small">
                                        <strong>Available Until:</strong> <?= esc($q['end_date'] ? date('F j, Y, g:i a', strtotime($q['end_date'])) : 'No Deadline') ?> &bull; 
                                        <strong>Time Limit:</strong> <?= esc($q['time_limit'] ?: 'None') ?> mins
                                    </p>
                                </div>
                                <?php if (!empty($q['attempts'])): ?>
                                    <span class="badge bg-success rounded-pill px-3 py-2"><i class="bi bi-check-circle me-1"></i> Attempted (<?= count($q['attempts']) ?>/<?= esc($q['max_attempts']) ?>)</span>
                                <?php else: ?>
